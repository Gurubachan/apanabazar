...
<script src="<?=base_url('assets/js/load_function.js')?>"></script>
    ...
    $('#frmSubcategory').submit(function (e) {
        e.preventDefault();
    ...
    function load_report() {
        $.ajax({
            type:'post',
            url:"<?=base_url('Subcategory/report_subcategory')?>",
            dataType:'json',
            success :function (data) {
                if (data.status!=false){
                    ...
                    jQuery("#showReportsubcat").html(htmldata);
                }
            }
        })
    }
    function active_inactive(rowid,isactive) {
        active_inactive_records(rowid,isactive,"<?=base_url('Subcategory/isactive_subcategory')?>");
    }
