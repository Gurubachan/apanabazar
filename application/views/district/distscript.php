...
<script type="text/javascript">
    ...
    function load_report() {
        $.ajax({
            type:'post',
            url:"<?=base_url('District/report_dist')?>",
            dataType:'json',
            success :function (data) {
                if (data.status!=false){
                    ...
                    jQuery("#showReportDist").html(htmldata);
                }
            }
        })
    }
    function active_inactive(rowid,isactive) {
        active_inactive_records(rowid,isactive,"<?=base_url('District/isactive_dist')?>");
    }
</script>
