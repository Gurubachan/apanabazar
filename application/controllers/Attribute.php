...
class Attribute extends CI_Controller
{
    ...
    public function index()
    {
        try{
        	if(isset($this->session->adminLogin['userid'])){
				...
				$this->load->view('Attribute/frmAttribute');
				$this->load->view('include/footer');
				$this->load->view('Attribute/attribute_script');
			}else{
        		redirect('Welcome/');
			}
        }catch (Exception $e){
    ...
    public function create_attribute()
    {
        try {
            ...
            if (isset($request->txtAttribute) && preg_match("/[a-zA-Z ]{3,50}$/", $request->txtAttribute)) {
                $insert[0]['name'] = $request->txtAttribute;
            } else {
                $status = false;
                $data['data'] = "Attribute Name Error";
            }
            if (isset($request->cboAttributegroup) && is_numeric($request->cboAttributegroup)) {
                $insert[0]['attributegroupid'] = $request->cboAttributegroup;
            } else {
                $status = false;
                $data['data'] = "Attribute-group Error";
            }
            if($status){
                if(isset($request->txtid) && is_numeric($request->txtid)){
                    if($request->txtid>0){
                        if(isset($this->session->editForm['id']) && $this->session->editForm['id']==$request->txtid){
                            ...
                            $res=$this->Model_Db->update(18,$insert,"id",$request->txtid);
                            if($res!=false){
                        ...
                        $res=$this->Model_Db->insert(18,$insert);
                        if($res!=false){
    ...
    public function report_attribute()
    {
        try {
            $data = array();
            $request = json_decode(json_encode($_POST), false);
    ...
    public function edit_attribute(){
        try{
            ...
            if(isset($request->rowid) && is_numeric($request->rowid) && $request->rowid>0) {
                ...
                $res = $this->Model_Db->select(18, null, $where);
                if ($res != false){
                    $data['status'] = true;
                    foreach ($res as $r) {
                        $data['data'][] = array(
    ...
    public function isactive_attribute(){
        try{
            ...
            if(isset($request->rowid) && is_numeric($request->rowid) && $request->rowid>0){
                ...
                $res=$this->Model_Db->select(18,null,$where);
                if($res!=false){
                    $result=$this->Model_Db->update(18,$update,'id',$request->rowid);
                    if($result!=false){
                        $data['status']=true;
