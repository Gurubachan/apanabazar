...
class Variant extends CI_Controller
{
    ...
    public function index()
    {
        try{
        	if(isset($this->session->adminLogin['userid'])){
				...
				$this->load->view('Variant/frmVariant');
				$this->load->view('include/footer');
				$this->load->view('Variant/variant_script');
			}else{
        		redirect('Welcome/');
			}
        }catch (Exception $e){
    ...
    public function create_variant()
    {
        try {
            ...
            if (isset($request->txtVariant) && preg_match("/[a-zA-Z ]{1,50}$/", $request->txtVariant)) {
                $insert[0]['variantname'] = $request->txtVariant;
            } else {
                $status = false;
                $data['data'] = "Variant Name Error";
            }
            if($status){
                if(isset($request->txtid) && is_numeric($request->txtid)){
                    if($request->txtid>0){
                        if(isset($this->session->editForm['id']) && $this->session->editForm['id']==$request->txtid){
                            ...
                            $res=$this->Model_Db->update(28,$insert,"id",$request->txtid);
                            if($res!=false){
                        ...
                        $res=$this->Model_Db->insert(28,$insert);
                        if($res!=false){
    ...
    public function report_variant()
    {
        try {
            $data = array();
            $request = json_decode(json_encode($_POST), false);
    ...
    public function load_variant()
    {
        try {
            ...
            if ($res != false) {
                foreach ($res as $r) {
                    $data[] = "<option value='$r->id'>$r->variantname</option>";
                }
            }
            echo json_encode($data);
        } catch (Exception $e) {
