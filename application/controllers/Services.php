	...
	public function index()
	{
		try{
			if(isset($this->session->adminLogin['userid'])){
				...
				$this->load->view('services/service_script');
			}else{
				redirect('Welcome/');
			}
		}catch (Exception $e){
...
//            }
            if (isset($request->txtService) && preg_match("/[a-zA-Z ]{3,50}$/", $request->txtService)) {
                $insert[0]['serviceName'] = $request->txtService;
            } else {
                $status = false;
                $data['data'] = "Service Name Error";
            }
            if($status){
