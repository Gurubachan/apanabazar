...
class State extends CI_Controller
{
    ...
    public function index()
    {
        try{
			if(isset($this->session->adminLogin['userid'])){
				...
				$this->load->view('state/frmState');
				$this->load->view('include/footer');
				$this->load->view('state/state_script');
			}else{
				redirect('Welcome/');
			}
        }catch (Exception $e){
    ...
    public function create_state()
    {
        try {
            ...
            if (isset($request->txtStatename) && preg_match("/[a-zA-Z ]{3,10}$/", $request->txtStatename)) {
                $insert[0]['name'] = $request->txtStatename;
            } else {
                $status = false;
                $data['data'] = "State Name Error";
            }
            if (isset($request->txtCode) && is_numeric($request->txtCode)) {
                $insert[0]['code'] = $request->txtCode;
            } else {
                $status = false;
                $data['data'] = "State Code Error";
            }
            if($status){
                if(isset($request->txtid) && is_numeric($request->txtid)){
                    if($request->txtid>0){
                        if(isset($this->session->editForm['id']) && $this->session->editForm['id']==$request->txtid){
                            ...
                            $res=$this->Model_Db->update(22,$insert,"id",$request->txtid);
                            if($res!=false){
                        ...
                        $res=$this->Model_Db->insert(22,$insert);
                        if($res!=false){
    ...
    public function edit_state(){
        try{
            ...
            if(isset($request->rowid) && is_numeric($request->rowid) && $request->rowid>0) {
                ...
                $res = $this->Model_Db->select(22, null, $where);
                if ($res != false){
                    $data['status'] = true;
                    foreach ($res as $r) {
                        $data['data'][] = array(
    ...
    public function isactive_state(){
        try{
            ...
            if(isset($request->rowid) && is_numeric($request->rowid) && $request->rowid>0){
                ...
                $res=$this->Model_Db->select(22,null,$where);
                if($res!=false){
                    $result=$this->Model_Db->update(22,$update,'id',$request->rowid);
                    if($result!=false){
                        $data['status']=true;
