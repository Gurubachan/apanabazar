...
class Vendor_flag extends CI_Controller
{
    ...
    public function create_vendor_flag()
    {
        try {
            $data = array();
            $insert = array();
            ...
            if (isset($request->txtFlagname) && preg_match("/[a-zA-Z ]{3,50}$/", $request->txtFlagname)) {
                $insert[0]['flagname'] = $request->txtFlagname;
            } else {
                $status = false;
                $data['data'] = "flagname Error";
            }
            if($status){
    ...
    public function report_vendor_flag()
    {
        try {
            $data = array();
            $request = json_decode(json_encode($_POST), false);
    ...
    public function load_vendor_flag()
    {
        try {
            $data = array();
            $where = "isactive=true";
            ...
            if ($res != false) {
                foreach ($res as $r) {
                    $data[] = "<option value='$r->id'>$r->flagname</option>";
                }
            }
            echo json_encode($data);
        } catch (Exception $e) {
    ...
    public function isactive_vendor_flag(){
        try{
            ...
            if(isset($request->rowid) && is_numeric($request->rowid) && $request->rowid>0){
                ...
                $res=$this->Model_Db->select(8,null,$where);
                if($res!=false){
                    $result=$this->Model_Db->update(8,$update,'id',$request->rowid);
                    if($result!=false){
                        $data['status']=true;
